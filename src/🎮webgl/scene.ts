...
export class PingPongBuffer {
	 ...
	 */
	constructor(public renderer: ogl.Renderer, public width: number, public height: number) {
...
export class OGLFullScreenShader {
	...
	constructor(target: HTMLCanvasElement | ogl.RenderTarget, program: (renderer: ogl.Renderer) => ogl.Program ) {
		...
		this.mesh = new ogl.Mesh(this.renderer.gl, { geometry: this.triangle, program: program(this.renderer) })
	}
	...
	update(){
		this.renderer.render({ scene: this.mesh, camera: this.camera, target: this.target })
	}
}